...
import { formatearPrecio, type TarifasMasajes } from "@/lib/tarifas";
import PageHero from "@/components/ui/PageHero";
import ScrollReveal from "@/components/ui/ScrollReveal";
import VideoPlayer from "@/components/ui/VideoPlayer";

// Los precios vienen del documento de tarifas (editable desde el panel).
const treatments = [
  {
    id: "ventosas",
...
export default function MasajesContent({ masajes }: { masajes: TarifasMasajes }) {
  const t = useTranslations("Massage");
  const tCommon = useTranslations("Common");
  const locale = useLocale();
  const precioDe = (featured?: boolean) =>
    featured
      ? { price: formatearPrecio(masajes.premiumPrecio), memberPrice: formatearPrecio(masajes.premiumPrecioUsuarios) }
      : { price: formatearPrecio(masajes.precio), memberPrice: formatearPrecio(masajes.precioUsuarios) };
  const experiencePoints = [
    ...
    {
      value: `${formatearPrecio(masajes.precioUsuarios)}€`,
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {treatments.map((treatment, i) => {
              const { price, memberPrice } = precioDe(treatment.featured);
              return (
                    ...
                    <div className="flex items-end justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-white/70">
                          {t("memberPrice")}:{" "}
                          <span className="font-semibold text-white">{memberPrice}&euro;</span>
                        </p>
                      </div>
